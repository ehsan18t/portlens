...
Docker/Podman container awareness to PortLens. The goal is to transform raw
...
By default, PortLens shows only developer-relevant ports. A port is included
...
(e.g., `~/.config/portlens/frameworks.toml`) mapping process names,
